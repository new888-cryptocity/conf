...
var (
	...
	LogLevel             int   = 4           //日誌等級 範例=>目前0:全印 設置1:不顯示debug訊息 ( 0:調試信息(全部列印) 1:輸出日誌信息 2:警告 3:錯誤 4:嚴重錯誤 )
	ConfigDirName              = "config"    //配置文件目錄名
	EVTCHANSIZE                = 200       //事件通道大小
	MSGPACKERBUF         int32 = 1024 * 100 //消息緩沖區大小
	READBUFLEN           int   = 1024 * 100 //接收消息緩沖區大小
	DefaultEventThrottle int   = 100       //預設的事件並發數
	ClearLog             bool  = false
)
...
var (
	...
	TCPSERVERCONN_HALL_CLIENT     = 10 //大廳客戶端連接最大連接數
	TCPSERVERCONN_HALL_GAMESERVER = 20  //大廳遊戲服務器連接最大連接數
	TCPSERVERCONN_ROOM_CLIENT     = 20  //遊戲客戶端連接最大連接數

	TCPSERVERCONN_DBSERVER = 50 //dbserverTCP連接數

	TCPSERVERCONN_NOTIFYSERVER = 50             //notifyserver tcp連接數
